...
// const int SPMathcer::TH_HIGH = 100;
    ...
    int SPMatcher::SearchForTriByEpi(
        ...
        cv::Mat grid_for_search(camera_height, camera_width, CV_8UC1);
        ...
        for (size_t i = 0; i < pKF1->N; i++) {
            ...
            for (size_t i_iter = 0; i_iter < iter.count; i_iter++, iter++) {
                ...
                if (cell_idx.y < camera_height / 8 && cell_idx.x < camera_width / 8) {
            ...
            for (auto &&cell_idx : cells_to_check) {
                ...
                int16_t idx;//TODO my = pKF2->occ_grid.at<int16_t>(cell_idx.second);
                if (idx != -1) {
                    // cv::Scalar color;
...
//            const cv::Mat MPdescriptor = pMP->getDescTrack();
            ...
            for (vector<size_t>::const_iterator vit = vIndices.begin(),
                ...
                // TODO my
                const float dist = 0;// = DescriptorDistance(MPdescriptor, d);
...
//        const auto sigma = pKF2->cov2_inv_[idx];
        const float sigma2_inv_x = 0;// = sigma.x();
        const float sigma2_inv_y = 0;//= sigma.y();

...
const int ORBmatcher::TH_HIGH = 300;
...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        const int &nPredictedLevel = 0;//pMP->mnTrackScaleLevel;
...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        int nPredictedLevel = 0;//pMP->PredictScale(dist,pKF);
 ...
 * @return 匹配数量
 */
int ORBmatcher::SearchForInitializationWithGMS(Frame &Frame_1, Frame &Frame_2, vector<int> &vnMatches12)
{
    int nmatches=0;
    int nbestMatches=0;
    ...
    //resize()
    vector<cv::KeyPoint> kp1 = Frame_1.mvKeysUn;
    vector<cv::KeyPoint> kp2 = Frame_2.mvKeysUn;
    cv::Mat d1 = Frame_1.mDescriptors;
    cv::Mat d2 = Frame_2.mDescriptors;
    // 1. 通过 bf 进行匹配
    vector<cv::DMatch> matches_all;//,matches_gms;
    cv::BFMatcher matcher(cv::NORM_L2);
    matcher.match(Frame_1.mDescriptors, Frame_2.mDescriptors, matches_all);

    cv::Mat show = Frame_2.img;
    cv::cvtColor(show, show, CV_GRAY2BGR);
    for(unsigned int i=0; i<matches_all.size(); i++)
    {
        //if(matches_all[i]>=0)
        {
            cv::line(show, Frame_1.mvKeysUn[matches_all[i].queryIdx].pt, Frame_2.mvKeysUn[matches_all[i].trainIdx].pt,
                     cv::Scalar(0,255,0));
        }
    }
//    cv::Mat show;
//    cv::drawMatches(Frame_1.img, Frame_1.mvKeysUn, Frame_2.img, Frame_2.mvKeysUn, matches_all, show);
    cv::imshow("show", show);
//    cv::waitKey(2);
...
int ORBmatcher::SearchByGMS(KeyFrame *pKF, Frame &F, vector<MapPoint *> &vpMapPointMatches)
{
    int nmatches=0;
    int inliers = 0;
    vpMapPointMatches = vector<MapPoint*>(F.N,static_cast<MapPoint*>(NULL));
...
int ORBmatcher::SearchByGMS(KeyFrame *pKF1, KeyFrame* pKF2, std::vector<MapPoint*> &vpMatches12)
{
...
int ORBmatcher::SearchForTriangulationGMS(KeyFrame *pKF1, KeyFrame* pKF2, cv::Mat F12,

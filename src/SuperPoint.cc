...
cv::Mat SPdetect(std::shared_ptr<SuperPoint> model, cv::Mat img, std::vector<cv::KeyPoint> &keypoints, double threshold, bool nms, bool cuda)
{
    ...
    auto grid = torch::zeros({1, 1, kpts.size(0), 2}).cuda();  // [1, 1, n_keypoints, 2]
...
void SPDetector::detect(torch::Tensor &x, bool cuda)
{
//    auto x = torch::from_blob(img.data, {1, 1, img.rows, img.cols}, torch::kByte).clone();
    x = x.to(torch::kFloat) / 255;
...
//    auto grid = torch::zeros({1, 1, fkpts.size(0), 2});
    auto grid = torch::zeros({1, 1, fkpts.size(0), 2}).cuda();  // [1, 1, n_keypoints, 2]
